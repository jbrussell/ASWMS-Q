...
for i=2:m-1
    for j=2:n-1
        hkm = vdist(xi(i-1,j),yi(i-1,j),xi(i+1,j),yi(i+1,j))/1e3/2;
        hdeg = km2deg(hkm);
        h1=hdeg*pi/180;
...
% 		h1=km2deg(vdist(xi(i-1,j),yi(i-1,j),xi(i+1,j),yi(i+1,j))/1e3)*pi/180;
		d2Alat(i,j)=(dAlat(i+1,j)*cosd(xi(i+1,j))-dAlat(i-1,j)*cosd(xi(i-1,j)))/h1/2/R^2/cosd(xi(i,j));
        
        hkm = vdist(xi(i,j-1),yi(i,j-1),xi(i,j+1),yi(i,j+1))/1e3/2;
        hdeg = km2deg(hkm);
        h2=hdeg*pi/180;
% 		h2=km2deg(vdist(xi(i,j-1),yi(i,j-1),xi(i,j+1),yi(i,j+1))/1e3)*pi/180;
		d2Alon(i,j)=(A(i,j+1)-2*A(i,j)+A(i,j-1))/h2.^2/R^2/cosd(xi(i,j))^2;
	end
end
d2A = d2Alat + d2Alon;
