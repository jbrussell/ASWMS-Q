%% This script is similar to stack_helm.m, but adding azimuthal anisotropy inversion into the result.
%
% written by Ge Jin, LDEO
%
% This version assumes a 1-D velocity structure (i.e., inputs measurements
% from a9_a_helmholtz_eq) and includes weighting based on number of good
...
phase_v_path_eik = [workingdir,'eikonal/'];
...
for ie = 1:length(phvmatfiles)
    ...
    temp = load([phase_v_path_eik,'/',helmholtz(1).id,'_eikonal_',comp,'.mat']);
	eventphv = temp.eventphv;
	if is_offgc_propagation
        eikonl_propazi_output_path = [workingdir,'eikonal_propazi/'];
        temp = load([eikonl_propazi_output_path,'/',helmholtz(1).id,'_eikonal_',comp,'.mat']);
        eventphv_propazi = temp.eventphv;
    end
	disp(helmholtz(1).id);
	evla(ie) = helmholtz(ip).evla;
	evlo(ie) = helmholtz(ip).evlo;
	for ip=1:length(periods)
        if helmholtz(ip).goodnum<min_goodnum || ...
            helmholtz(ip).Mw<min_Mw 
                    % eventphv(ip).evdp>max_evdp
            continue;
        end
        GV_mat(:,:,ie,ip) = helmholtz(ip).GV_cor;
		...
		azi = rad2deg(azi);
		azi_mat(:,:,ie,ip) = azi;
        weights(ie,ip) = helmholtz(ip).goodnum.^(-1/2); % jbr;
	end
...
filename = [workingdir,'helmholtz_stack_aniso1D_',comp,'.mat'];
