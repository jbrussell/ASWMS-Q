...
%
% Gammacorr: This version includes an additional correction term "gamma", which
% is the ratio of structural phase velocity over apparent phase velocity and 
% premultiplies the term containing beta on the left hand side of eq (4) of 
% Bao et al. (2016) GJI. This term accounts for finite-frequency effects. The
% azimuthally averaged gamma is close to 1 and therefore was previously ignored.
%
...
for ip = 1:length(avgphv)
    clear ampgradR_ampnorm_dot_tpgrad amp_gradR_ampnorm_map amp_gradR_map amp_gradT_map amp_gradlat_ampnorm_map amp_gradlon_ampnorm_map ampgrad_dot_tpgrad_ampnorm amp_term amp_term_err azi amp_decay_map tp_focus_map tp_grad_map amp_grad_map ampgrad_dot_tpgrad amp_grad_norm_map evids dist_map amp_map amp_gradlat_map amp_gradlon_map tp_gradlat_map tp_gradlon_map azi_anom phv_map gamma_map
    evcnt = 0;
...
%         azi_prop(azi_prop<0) = azi_prop(azi_prop<0)+360;
        ...
        for ii=1
            ...
            tp_focus = gridfit_jg_geo(xi(:),yi(:),tp_focus(:),xnode,ynode,...
                                'smooth',floor(smD./deg2km(gridsize)),'regularizer','laplacian','solver','normal')';
            amp_decay_err = gridfit_jg_geo(xi(:),yi(:),amp_decay_err(:),xnode,ynode,...
                                'smooth',floor(smD./deg2km(gridsize)),'regularizer','laplacian','solver','normal')';
            tp_focus_err = gridfit_jg_geo(xi(:),yi(:),tp_focus_err(:),xnode,ynode,...
                                'smooth',floor(smD./deg2km(gridsize)),'regularizer','laplacian','solver','normal')';
        end
...
%     plot(res(:),azi_anom(:),'o');
    ...
    [~, alpha, dlnbeta_dx, dlnbeta_dy]=fit_alpha_beta_gammacorr(azi(:),gamma_map(:),amp_term(:));
...
%     amp_term(Ibad) = nan;
    ...
    [~,Isort] = sort(azi(:));
    azi_srt = azi(Isort);
    gamma_srt = gamma_map(Isort);
    amp_term_srt = amp_term(Isort);
    bins = [0:azi_bin_deg:360];
    amp_bin = 0; azi_bin = 0; amp_bin_std = 0; azi_bin_err = 0; gamma_bin = 0;
    for ibin = 1:length(bins)-1
        ...
        gamma_bin(ibin) = nanmedian(gamma_srt(I_bin));
    end
    ...
    attenuation(ip).gamma_map = gamma_map;
...
%     [alpha, dlnbeta_dx, dlnbeta_dy, beta_tau, azi_maxamp, alpha_err, dlnbeta_dx_err, dlnbeta_dy_err, beta_tau_err, azi_maxamp_err]=run_fit_alpha_beta_bootstrap(azi(:),amp_term(:),amp_term_err(:),Nbs);
    [alpha, dlnbeta_dx, dlnbeta_dy, beta_tau, azi_maxamp, alpha_err, dlnbeta_dx_err, dlnbeta_dy_err, beta_tau_err, azi_maxamp_err]=run_fit_alpha_beta_bootstrap_randresamp_gammacorr(azi,gamma_map,amp_term,amp_term_err,Nbs);
    ...
    [alpha, dlnbeta_dx, dlnbeta_dy, beta_tau, azi_maxamp, alpha_err, dlnbeta_dx_err, dlnbeta_dy_err, beta_tau_err, azi_maxamp_err]=run_fit_alpha_beta_bootstrap_gammacorr(azi_bin(:),gamma_bin(:),amp_bin(:),amp_bin_std(:),Nbs);
...
%             azis = squeeze(azi(ix,iy,:));
            if length(find(~isnan(amps)))>N_min_evts
                Nbs = 1;
%                 [alpha, dlnbeta_dx, dlnbeta_dy, beta_tau, azi_maxamp, alpha_err, dlnbeta_dx_err, dlnbeta_dy_err, beta_tau_err, azi_maxamp_err]=run_fit_alpha_beta_bootstrap(azis(:),amps(:),amps_err(:),Nbs);
                [para, alpha, dlnbeta_dx, dlnbeta_dy]=fit_alpha_beta_gammacorr(azis(:),gammas(:),amps(:),amps_err(:));
...
%% Plot gamma correction term

figure(102); clf;
set(gcf,'Position',[616    71   850   947]);
sgtitle('\gamma = c/c''','fontweight','bold','fontsize',18);
for ip = 1:length(attenuation)
    subplot(M,N,ip)
    gamma_map_evs = squeeze(nanmean(nanmean(attenuation(ip).gamma_map,1),2));
    azi_evs = squeeze(nanmean(nanmean(attenuation(ip).azi,1),2));
    plot(attenuation(ip).azi(:),(attenuation(ip).gamma_map(:)),'.b'); hold on;
    plot(azi_evs,gamma_map_evs,'.r'); hold on;
    plot([min(attenuation(ip).azi(:)) max(attenuation(ip).azi(:))],[1 1],'-k','linewidth',1);
    plot([min(attenuation(ip).azi(:)) max(attenuation(ip).azi(:))],[1 1]*nanmedian(attenuation(ip).gamma_map(:)),'--g','linewidth',2);
    title([num2str(attenuation(ip).period),' s'])
    xlabel('azimuth');
    ylabel('\gamma');
end
...
for ip = 1:length(attenuation)
    subplot(M,N,ip)
    gamma_map_evs = squeeze(nanmean(nanmean(attenuation(ip).gamma_map,1),2));
    edges = min(attenuation(ip).gamma_map(:)):0.005:max(attenuation(ip).gamma_map(:));
    histogram(attenuation(ip).gamma_map(:),edges,'facecolor','b'); hold on;
    histogram(gamma_map_evs,edges,'facecolor','r','facealpha',0.5); hold on;
    yvals = get(gca,'ylim');
    plot([1 1],yvals,'-k','linewidth',1);
    plot([1 1]*nanmedian(attenuation(ip).gamma_map(:)),yvals,'--g','linewidth',2);
    text(0.05,0.9,['std: ',num2str(nanstd(attenuation(ip).gamma_map(:)))],'Units','normalized');
    title([num2str(attenuation(ip).period),' s'])
    ylim(yvals);
    xlabel('\gamma');
    ylabel('counts');
