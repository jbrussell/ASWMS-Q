...
% 2013.1.16
%
% JBR 2022/5 - modified to solve for maps of group velocity
%
clear
...
% output path
eikonl_output_path = [workingdir,'eikonal_grv/'];
...
%for ie = 30
	...
	matfilename = [eikonl_output_path,'/',eventcs.id,'_eikonal_grv_',comp,'.mat'];
	...
	for ip = 1:length(periods)
	    ...
	    if is_offgc_smoothing==1
	        phase_lat = -aziprop.eventgrv(ip).GVx; % phase slowness in x-direction
	        phase_lon = -aziprop.eventgrv(ip).GVy; % phase slowness in y-direction
	...
	%         v = trace(D);
		...
		eventgrv(ip).rays = rays;
		eventgrv(ip).w = diag(W);
		eventgrv(ip).goodnum = length(find(eventgrv(ip).w>0));
		eventgrv(ip).badnum = length(find(eventgrv(ip).w==0));
		eventgrv(ip).dt = dt;
	    eventgrv(ip).dt_res = dt_res; % data residuals
	    eventgrv(ip).chi2 = chi2; % chi2 misfit
		eventgrv(ip).GV = GV;
		eventgrv(ip).GVx = GVx;
		eventgrv(ip).GVy = GVy;
	    eventgrv(ip).phv_err = phv_err;
	    eventgrv(ip).phvx_err = phvx_err;
	    eventgrv(ip).phvy_err = phvy_err;
	    eventgrv(ip).dtau_err = dtau_err;
	    eventgrv(ip).dtaux_err = dtaux_err;
	    eventgrv(ip).dtauy_err = dtauy_err;
		eventgrv(ip).phv_fwd = phv_fwd;
		eventgrv(ip).raydense = raydense;
		eventgrv(ip).lalim = lalim;
		eventgrv(ip).lolim = lolim;
		eventgrv(ip).gridsize = gridsize;
		eventgrv(ip).id = eventcs.id;
		eventgrv(ip).evla = eventcs.evla;
		eventgrv(ip).evlo = eventcs.evlo;
		eventgrv(ip).evdp = eventcs.evdp;
		eventgrv(ip).period = periods(ip);
		eventgrv(ip).traveltime = travel_time(ip).tp;
		eventgrv(ip).stlas = eventcs.stlas;
		eventgrv(ip).stlos = eventcs.stlos;
		eventgrv(ip).stnms = eventcs.stnms;
		eventgrv(ip).isgood = eventgrv(ip).w>0;
		eventgrv(ip).Mw = eventcs.Mw;
		disp(['Period:',num2str(periods(ip)),', Goodnum:',num2str(eventgrv(ip).goodnum),...
				'Badnum:',num2str(eventgrv(ip).badnum)]);
	...
	if isfigure
		...
		for ip = 1:length(periods)
			...
			set(ax, 'Visible', 'off')
			h1=surfacem(xi,yi,eventgrv(ip).GV);
			% set(h1,'facecolor','interp');
	%			load pngcoastline
	%			geoshow([S.Lat], [S.Lon], 'Color', 'black','linewidth',2)
			title(['Periods: ',num2str(periods(ip))],'fontsize',15)
			avgv = nanmean(eventgrv(ip).GV(:));
			if isnan(avgv)
				continue;
			end
			r = 0.1;
