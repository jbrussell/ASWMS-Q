...
%     amp_term(Ibad) = nan;
    ...
    [para2, alpha2, beta_tau2, azi_maxamp2]=fit_alpha_beta_Lin12(azi_bin(:),amp_bin(:));
    beta_tau = sqrt(dlnbeta_dx.^2+dlnbeta_dy.^2);
    azi_maxamp = atan2d(dlnbeta_dx,dlnbeta_dy);
    dlnbeta_dx_err = (parastd(2,1)-parastd(1,1))/2;
    dlnbeta_dy_err = (parastd(2,2)-parastd(1,2))/2;
...
%             azis = squeeze(azi(ix,iy,:));
            if length(find(~isnan(amps)))>N_min_evts
                ...
                [para2, alpha2, beta_tau2, azi_maxamp2]=fit_alpha_beta_Lin12(azis,amps);
                beta_tau = sqrt(dlnbeta_dx.^2+dlnbeta_dy.^2);
                azi_maxamp = atan2d(dlnbeta_dx,dlnbeta_dy);
                parastd=confint(para,.95);
                parastd2=confint(para2,.95);
