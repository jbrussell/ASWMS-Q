...
c      integer*4 gpr_$light_blue, gpr_$light_purple
...
c
c      c_value(1) = gpr_$black
c      c_value(2) = gpr_$white
c
c     mode = 'off'
c
c      c_value(3) = gpr_$blue
c   
...
c     mode = highlighted'
c
c      c_value(10) = gpr_$green
c
c     white
c
      do ii = 11, 16
c        c_value(ii) = gpr_$white
      end do
c
      radius = 1
      unit = 1
...
c
     ...
     +         ii=1,isearch)
      ...
      write(9,*) isearch, (index(ii),(value(index(ii),jj),jj=1,2),
     +         ii=1,isearch)     
      write(4) gv0, s0
      write(9,*) gv0, s0
      do ii = nlow, nup
        write(4) ii,(fill(inx(jj,ii)), jj = llow, lup)
        write(9,*) ii,(fill(inx(jj,ii)), jj = llow, lup)
      end do
      close(4)
      close(9)
      go to 9010
