...
c
c JBR: Modified slightly so that we can use our old Matlab input 
c files exactly as they were
c
CAD !!!!!!!!!
CAD VERY IMPORTANT !!!!!!!
CAD I MODIFIED THE .EIG FILE TO HAVE HEADER LINES
CAD SO THAT IT CAN WORK WITH OTHER CODES LIKE IDA_GRN
CAD
c
...
c JBR added 7/20/21      
     ...
     + form='formatted',iostat=iret)
      if(iret.ne.0) then
         print *,' error opening mode file : ',filnam(1:lnblnk(filnam))
         stop
      end if
      call wtable(9,8,3,ifreq)
      close(9)
C      call wtable(8,3,ifreq)
...
c JBR added 7/20/21 
C      subroutine wtable(iout,ioeig,ifreq)
      subroutine wtable(iin,iout,ioeig,ifreq)
c JBR end of addition 
...
cad added 6/10/21
      real*4 tdum(mk,9),wminl, wmaxl, wgravl, trefl 
      common/rindx/nic,noc,nsl,nicp1,nocp1,nslp1,n
      common/head/ tdum
cad end of addition 6/10/21      
      dimension wt(2)
      data inss/5/
      cmhz=pi/500.d0
      stepf=1.d0        
...
C      read(*,*) lmin,lmax,wmin,wmax,normin,normax
      read(iin,*) lmin,lmax,wmin,wmax,normax
      nmx=9999
      if(normax.le.0) normax=nmx
      normax=normax-1
      normin=0
c JBR end of addition
c MB added one line below
...
cad change 6/10/21
c      dimension abuf(6*mk+5)
      dimension abuf(6*mk+8)
cad end of change
      equivalence (nn,abuf)
      nn=nord
      ll=l
...
c        nvec=6*n+5
        nvec=6*n+8
cad end of change
cad 6/11/21 added the factor of -1*a below but not sure why I need it
c        do i=1,n
c          buf(i)=a(1,i)
c          buf(i+n)=a(2,i)
c          buf(i+n+n)=a(3,i)
...
c        enddo
	...
	do i=1,nocor
	  buf(i)=a(1,noc+i)
	  buf(i+nocor)=a(2,noc+i)
	enddo	     
      elseif(jcom.eq.1)then
        stop('HAVE NOT FIXED THIS FOR RADIAL MODES')
cad end of change
...
cad added this 6/10/21 cccccccc
        do i = 1, n
          tdum(i,1) = r(i)
          tdum(i,2) = rho(i)
          tdum(i,3) = vpv(i)
...
cad end of addition 6/10/21 ccccccccc
