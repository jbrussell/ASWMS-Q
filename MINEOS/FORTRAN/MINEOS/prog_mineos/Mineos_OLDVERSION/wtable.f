...
c
   ...
   15 KE(I)=-10
      ...
      kmod=kmod+1
c
c JRB      print*,'WT(I), = ',WT(I)
c JBR      print*,'WT(2), = ',WT(2)
c JBR DEBUG
c JBR     IF(WT(I).NE.WT(I)) THEN
c JBR       print*,'I = ',I
c JBR     ENDIF
...
c      go to 4444
      if (icount .gt. 10*nmode) then
        ...
        print*,' icount, nmode:   ',icount,nmode
c
...
C*** FIND ROOTS ***
      ...
      MAXO=8
c
c JBR      print*,'WT(2) before = ',WT(2)
      CALL ROTSPL(NEV,EPS1,WMIN,WMAX,WT,IOUT,IOEIG)
c JBR      print*,'WT(2) after = ',WT(2)
c
      GOTO 10
c
      END
