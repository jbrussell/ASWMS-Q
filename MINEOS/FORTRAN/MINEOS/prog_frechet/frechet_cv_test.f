      program frechet_cv_test
c
c     program to test frechet kernels for phase velocity
c
...
C23456789112345678921234567893123456789412345678951234567896123456789712
c
c      Updated 5/12/2015 to test scaling on phase velocity kernels
c      NJA
c
c
...
c
      ...
      print*, 'Discontinuities in the Frechet file:'
      do i = 1, ndisc
         print*, i, rad (kntdsc (i)) / 1000.0
      end do
c
c     open perturbed model file
c
      print*,' enter name of perturbed model file'
      read(*,'(a)') o_file
      open(1,file=o_file)
c
      if(ifanis.eq.0) then 
...
c     
        ...
        read(1,*) nmod
        do i = 1, nmod
          read(1,'(f8.0,3f9.2)') r1(i), d1(i), a1(i), b1(i)
        end do
        close(1)
c
c       interpolate these values
c
        call interpol(1,nmod,r1,d1,md1,bd1)
        call interpol(1,nmod,r1,a1,ma1,ba1)
        call interpol(1,nmod,r1,b1,mb1,bb1)
c
c       calculate model perturbation
c
        rl = rad(1)
        do i = 1, ksave
          r = rad(i)
          a = interple(1,nmod,r1,r,rl,a1,ma1,ba1)
          b = interple(1,nmod,r1,r,rl,b1,mb1,bb1)
          d = interple(1,nmod,r1,r,rl,d1,md1,bd1)
          dalpha(i) = a - alpha(i)
          dbeta(i) = b - beta(i)
          drho(i) = d - dn(i)
          rl = r
...
c
c       anisotropic section--just needs to handle more velocities
c
        read(1,*) 
        read(1,*) 
        read(1,*) nmod
        do i = 1, nmod
          read(1,'(f8.0,3f9.2,2f9.1,2f9.2,f9.5)') r1(i),d1(i),
     &          a1(i),b1(i),qk,qs,ah1(i),bh1(i),et1(i)
          if(ah1(i).eq.0. .and. bh1(i).eq.0.) then
             ah1(i)=a1(i)
             bh1(i)=b1(i)
             et1(i)=1.0
          end if
...
c
        ...
        call interpol(1,nmod,r1,a1,ma1,ba1)
        call interpol(1,nmod,r1,b1,mb1,bb1)
        call interpol(1,nmod,r1,ah1,mah1,bah1)
        call interpol(1,nmod,r1,bh1,mbh1,bbh1)
        call interpol(1,nmod,r1,et1,met1,bet1)
c
c       calculate model perturbation
c
        rl = rad(1)
        do i = 1, ksave
          ...
          b = interple(1,nmod,r1,r,rl,b1,mb1,bb1)
          d = interple(1,nmod,r1,r,rl,d1,md1,bd1)
          ah = interple(1,nmod,r1,r,rl,ah1,mah1,bah1)
          bh = interple(1,nmod,r1,r,rl,bh1,mbh1,bbh1)
          et = interple(1,nmod,r1,r,rl,et1,met1,bet1)
          dalpha(i) = a - alpha(i)
          dbeta(i) = b - beta(i)
          dalphah(i) = ah - alphah(i)
          dbetah(i) = bh - betah(i)
          deta(i) = et - eta(i)
          drho(i) = d - dn(i)
          rl = r
...
c                        
      print*, 'Discontinuities in the perturbed model:'
      npdisc = 0
      do i = 1, nmod
         pdr(i) = r1 (i+1) - r1 (i)
c
...
c
         if (abs (pdr (i)) .le. rfrctn) then
            npdisc = npdisc + 1
            kntpdsc (npdisc) = i
            print*, npdisc, r1 (i) / 1000.0
         else
         end if
      end do
c        
      skp = 0
      if (npdisc .eq. ndisc) then
         print*, 'Frechet and Perturbation files have the same number of
     & discontinuities.'
         do i = 1, ndisc
            dh (i) = r1 (kntpdsc (i)) - rad (kntdsc (i))
         end do
      else if (npdisc .gt. ndisc) then
         maxskp = npdisc - ndisc
         print*, 'Perturbation file has ', maxskp, ' more discontinuitie
     &s'
         i = 1
         do while (i .le. ndisc)
 10         dh (i) = r1 (kntpdsc (i + skp)) - rad (kntdsc (i))
            if (abs (dh (i)) .gt. maxdh) then
c
c           these discontinuities do not match
c
               print*, 'Warning: skipping a discontinuity.'
               print*, 'The Frechet & Perturbation radii (km) are :', 
     &                  rad (kntdsc (i)) / 1000.0, 
     &                  r1 (kntpdsc (i + skp)) / 1000.0
               skp = skp + 1
               if (skp .gt. maxskp) then
                  print*, 'Discontinuity structure incompattible.'
                  stop
               else
                  goto 10
               end if
      ...
      else if (ndisc .gt. npdisc) then
         maxskp = ndisc - npdisc
         print*, 'Frechet file has ', maxskp, ' more discontinuities'
         print*, 'This situation is not implemented in this code.'
         stop
      end if
c    
      if (ifanis.eq.0) then  
...
c                             
        do i = 1, ndisc
           if (dh (i) .ne. 0) then
              k = 0
              do while (abs (rad (kntdsc (i) + k) - rad (kntdsc (i))) 
     &               .le. abs (dh (i)))
                 dalpha (kntdsc (i) + k) = 0.0
                 dbeta (kntdsc (i) + k) = 0.0
                 drho (kntdsc (i) + k) = 0.0
                 if (dh (i) .gt. 0) then
                    k = k + 1
                 else
...
c                             
     ...
     &               .le. abs (dh (i)))
                 ...
                 dbeta (kntdsc (i) + k) = 0.0
                 drho (kntdsc (i) + k) = 0.0
                 dalphah (kntdsc (i) + k) = 0.0
                 dbetah (kntdsc (i) + k) = 0.0
                 deta (kntdsc (i) + k) = 0.0
                 if (dh (i) .gt. 0) then
                    k = k + 1
                 else
...
c
c      print*,'rad,dpv,dsv,dph,dsh,dro,det'
c      do i = ksave - 40, ksave
c        print*,rad(i),dalpha(i),dbeta(i),dalphah(i),dbetah(i),
c     &         drho(i),deta(i)
c      end do
c      print*,'disc rad, dh'
c      do i = 1,ndisc
c       print*,rad(kntdsc(i)),dh(i)
c      end do
c
      k6 = 6*knot
      k5 = 5*knot
...
c        gv = w
c		 gv = cv
        dcv = 0.5*t*scale*scale
		cve = cv + dcv
		write(11,'(2i6,f8.2,2f8.3,f8.4)')
     &                 nn,ll,w*500./pi,cve,cv,dcv/cv*100.
	    irec=irec+1
      end do 
998   continue
      close(11)
