...
c
      ...
      character*(*) ia,ib
c
cccc these lines commented out 10/28/92  LSG
c     if(iargc(i).lt.n) go to 1
c     call getarg(n,ib)
c     go to 2
cccc end commented section
c
 1    if(ib.ne.' ') go to 2
      call query(ia,log)
      read(*,100)ib
...
c
cccc changed initialization for multiple calls
c
      do i = 1, jseg
        segmsk(i) = .true.
      end do
...
c   An 'all' keyword is easy as this is already the default.
 1    all=.false.
      if(no.eq.1.and.(phlst(1).eq.'all'.or.phlst(1).eq.'ALL')) then
        all=.true.
      endif
