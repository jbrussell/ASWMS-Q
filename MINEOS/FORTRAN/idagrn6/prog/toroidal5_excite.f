...
c               TOROIDAL
c
c Outputs excitation terms (jbr 07/10/18)
c
...
c
c a appears to be the excitation coefficient (jbr 07/10/18)
c
c***********************************************************************

      subroutine toroidal_h2_excite(lcomp,num_modes,ista,
     &                      del,azm,moment,index,lhetero,sta)
c
...
c
      ...
      m6 = dble(moment(6))
c
c OPEN EXCITATION ASCII OUTPUT FILE (jbr)
      print*,' Opening excitation file'
c      write(o_file,'(A11,A,A4)') 'excitation_',sta,'.asc'
      o_file = trim(sta)//".excite.asc"
...
c         JBR Save excitation to text file
          nn = scnl(j,1)
          ll = scnl(j,2)
          au = vecnl_h(j,5)
          write(3,10) nn,ll,a(j,1)*1e9,au
10        format(I3,I5,F20.8,F25.8)
          end do
      end if
c
c 
