      program idagrn6_excite
c Modified by jbr (09/01/21) to only output excitation text files.
c This version does not output SAC files
c
...
c     
      ...
      character*256 filep, fileq
      character*256 m_file, b_file, adump
c
...
c      read(11,'(a)') modeid    
     ...
     &        slon(ii), sele(ii)
c         print*, stname(ii), slat(ii), slon(ii), sele(ii)
      enddo

      lstat=.true.
c jbr Force excitation calculation
      lexcite=.true.
...
c      total = etime(utime1)    
         ...
         call source_strains_h2_jbr(source_file,d0,num_modes,ista,
     &        lread,lhetero)
...
cc....................................................
     ...
     &                     spheroidal modes'
                  ...
                  call spheroidal_exc_h2_jbr(lcomp,num_modes,ista,
     &                 del,azm,moment,lhetero,stname(ista))
               endif
            else
               if (lcomp .eq. 'R') then
                  print*,' no excitation for R comp and toroidal modes'
               else
                  call toroidal_exc_h2_jbr(lcomp,num_modes,ista,
     &                 del,azm,moment,lhetero,stname(ista))
               endif
            endif
         endif
...
c               call toroidal_h2(lcomp,num_modes,ista,
c     &              del,azm,moment,index2,lhetero)
               call toroidal_h2_excite(lcomp,num_modes,ista,
     &              del,azm,moment,index2,lhetero,stname(ista))
            endif
...
cc JBR COMMENT OUT ENTIRE SAC OUTPUT
cc....................................................
cc   WRITE SAC_FILE
cc
cc   if summation
cc....................................................
C         len1=lnblnk(sourid)
C         len2=lnblnk(stname(ista))
C         sacfnam=stname(ista)(1:len2)//"."//lcomp//".sac"
C
C         print*, 'stname: ', stname(ista)(1:len2)
C         print*, 'stla= ', slat(ista)
C         print*, 'stlo= ', slon(ista)
C         print*, 'dist= ', del(ista)
C         print*, 'azim= ', azm(ista)
C         call newhdr
C         call setnhv('NPTS',npts,nerr)
C         call setfhv('DELTA',real(dt),nerr)
C         call setfhv('B', 0.0, nerr)
C         call setfhv('EVLA',real(elat),nerr)
C         call setfhv('EVLO',real(elon),nerr)
C         call setfhv('EVDP',real(d0),nerr)
C         call setfhv('STLA',real(slat(ista)),nerr)
C         call setfhv('STLO',real(slon(ista)),nerr)
C         call setfhv('DIST',real(del(ista)),nerr)
C         call setfhv('AZ',real(azm(ista)),nerr)
C         call wsac0(sacfnam,rdump,data,nerr)
