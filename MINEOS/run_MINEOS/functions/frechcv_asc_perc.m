...
function [FRECH] = frechcv_asc_perc(TYPE,CARD,BRANCH)

% TYPE = 'T';
% CARD = param.CARDID;
% BRANCH = 0;

fid_log = fopen('draw_frechet_gv_perc.LOG','w'); %JBR -- write log file
...
for ip = 1:length(periods)
    ...
    com = sprintf('cat %s | draw_frechet_gv_perc',RUNFILE);
    [status,log] = system(com);
