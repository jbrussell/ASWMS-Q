    ...
    for i = 1:num_ll %loop over angular orders
        ...
        if ~isempty(eigen_ll)
            ...
            for j = first_nn:last_nn %loop over modes
                ...
                %eig.ll(i).nn(j+1).dat = eigen_ll_nn(:,:);
                eig.ll(i).nn(j+1).ll = eigen_ll_nn(1,1);
                eig.ll(i).nn(j+1).nn = eigen_ll_nn(1,2);
                eig.ll(i).nn(j+1).w = eigen_ll_nn(1,3);
                eig.r = eigen_ll_nn(:,4);
                eig.kappa = eigen_ll_nn(:,5);
                eig.mu = eigen_ll_nn(:,6);
                eig.rho = eigen_ll_nn(:,7);
                eig.ll(i).nn(j+1).u = eigen_ll_nn(:,8)*scale;
                eig.ll(i).nn(j+1).up = eigen_ll_nn(:,9)*scale;
                eig.ll(i).nn(j+1).v = eigen_ll_nn(:,10)*scale;
                eig.ll(i).nn(j+1).vp = eigen_ll_nn(:,11)*scale;
                eig.ll(i).nn(j+1).phi = eigen_ll_nn(:,12);
                eig.ll(i).nn(j+1).phip = eigen_ll_nn(:,13);
                eig.ll(i).nn(j+1).R = eigen_ll_nn(:,14);
                eig.ll(i).nn(j+1).S = eigen_ll_nn(:,15);
                eig.rad = 6371 - eig.r/1000;
                eig.ll(i).nn(j+1).per = 2*pi./eig.ll(i).nn(j+1).w;
            end
        else
            eig.ll(i).nn = [];
        end
    ...
    for i = 1:num_ll %loop over angular orders
        ...
        if ~isempty(eigen_ll)
            ...
            for j = first_nn:last_nn %loop over modes
                ...
                eig.mu = eigen_ll_nn(:,6);
                eig.rho = eigen_ll_nn(:,7);
                eig.ll(i).nn(j+1).wl = eigen_ll_nn(:,8)*scale;
                eig.ll(i).nn(j+1).wp = eigen_ll_nn(:,9)*scale;
                eig.ll(i).nn(j+1).T = eigen_ll_nn(:,10);
                eig.rad = 6371 - eig.r/1000;
                eig.ll(i).nn(j+1).per = 2*pi./eig.ll(i).nn(j+1).w;
            end
        else
