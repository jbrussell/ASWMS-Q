...
%   'XYZ' will use XYZ coords instead of Aki & Richards RTF
%
...
rtf = true;         % Use Aki & Richards convention
    ...
    for i = 1:nargin-4
        ...
        elseif ischar(varargin{i})
            switch varargin{i}
                ...
                case {'xyz','XYZ'}
                    rtf = false;    % use xyz coords instead of rtf 
            end 
        end
        end
    end     
...
if rtf
    ...
    if length(fm) == 6
        M(1,1) = fm(1); M(2,2) = fm(2); M(3,3) = fm(3);
        M(2,1) = fm(4); M(1,2) = M(2,1);
        M(3,1) = fm(5); M(1,3) = M(3,1);
        M(3,2) = fm(6); M(2,3) = M(3,2);
    else
        M = fm;
    end
end
...
if nc == 1       % 1 contour (closed unless single plane)
    ...
    % check for single plane
    d2 = (c(1,1)-c(end,1))^2 + (c(1,2)-c(end,2))^2;
    
    if d2 > 2 % single plane
        ang = atan2(c(end,2),c(end,1));     % location of contour end
        
        % check whther T axis to left or right of contour end for fill
        cw = true;
        ccheck = cross([c(end,1) c(end,2) 0],[vT(2) vT(1) 0]);
        if vT(3) < 0; ccheck(3) = -ccheck(3); end
        if ccheck(3) > 0; cw = false; end
...
% plot axes - don't do this as a rule, but useful when testing/debugging
if false
    if vT(3) >= 0
        text(centerX+sE*vT(2),centerY+sN*vT(1),'T','color',[0 1 0],'FontSize',12)
    else
        text(centerX-sE*vT(2),centerY-sN*vT(1),'T','color',[0 1 0],'FontSize',12)
    end
    if vB(3) >= 0
        text(centerX+sE*vB(2),centerY+sN*vB(1),'B','color',[0 0 1],'FontSize',12)
    else
        text(centerX-sE*vB(2),centerY-sN*vB(1),'B','color',[0 0 1],'FontSize',12)
    end
    if vP(3) >= 0
        text(centerX+sE*vP(2),centerY+sN*vP(1),'P','color',[1 0 0],'FontSize',12)
    else
        text(centerX-sE*vP(2),centerY-sN*vP(1),'P','color',[1 0 0],'FontSize',12)
    end
end

% add text to beachball
if ~isempty(ctitle)
    text(centerX,centerY+sN*1.25,ctitle,'HorizontalAlignment','center','FontSize',fsize)
end
